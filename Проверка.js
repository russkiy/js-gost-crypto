...
function Выполнить() {
	const
		...
		Разобрать_подпись = код => { const В_число = код => { let число = 0n; for (let сч = 0; сч < код.length; сч++) число += BigInt(Символы_кода_числа.indexOf(код[сч])) << BigInt(сч) * 7n; return число; }; return [ BigInt(В_число(код.substring(0, код.length >>> 1))), BigInt(В_число(код.substring(код.length >>> 1))) ].map(ч => ч.toString(16).toUpperCase().padStart(64, 0)); },
		Число_в_код = (число, длина = null) => { let строка = ""; while (число > 0n) { строка += Символы_кода_числа[число & 0x7Fn]; число >>= 7n; } if (длина !== null) строка = строка.padEnd(длина, Символы_кода_числа[0]); return строка; },
		Двойное_число_в_код = (r, s, длинный_хэш = false) => { r = Число_в_код(r, длинный_хэш ? 74 : 37), s = Число_в_код(s, длинный_хэш ? 74 : 37); return r + s; },
		...
		Проверочные_векторы = [
			...
			{	// ЭЦП ГОСТ Р 34.10-2018
				...
				Q_x: 0x7F2B49E270DB6D90D8595BEC458B50C58585BA1D4E9B788F6689DBD8E56FD80Bn,
				Q_y: 0x26F1B489D6701DD185C8413A977B3CBBAF64D1C593D26627DFFB101A87FF77DAn,
	...
	if (
		this.Шифрование.Простая_замена(
			this.Шифрование.Простая_замена("Проверка", Проверочные_векторы[0].ключ, undefined, false),
	...
	if (
		this.Шифрование.Гаммование(
			this.Шифрование.Гаммование("Проверка...", Проверочные_векторы[0].ключ, undefined, [0x508F7E55, 0x28FBC9BA]),
	...
	if (
		this.Шифрование.Гаммование_с_обратной_связью(
			this.Шифрование.Гаммование_с_обратной_связью("Проверка...", Проверочные_векторы[0].ключ, undefined, [0x508F7E55, 0x28FBC9BA], false),
	...
	const подпись = Разобрать_подпись(this.ЭЦП.Подписать(Проверочные_векторы[2].h, Число_в_код(Проверочные_векторы[2].d), this.ЭЦП.Наборы_параметров[Проверочные_векторы[2].параметры]));
	...
	if (
		this.ЭЦП.Проверить(
			...
			Двойное_число_в_код(...подпись.map(ч => BigInt("0x" + ч))),
			Двойное_число_в_код(Проверочные_векторы[2].Q_x, Проверочные_векторы[2].Q_y, false),
	...
	if (
		(ключ = this.ЭЦП.Согласование_ключей(
			Число_в_код(Проверочные_векторы[3].A_закр, 74),
			Двойное_число_в_код(...Проверочные_векторы[3].B_откр, true),
			Проверочные_векторы[3].UKM,
			this.ЭЦП.Наборы_параметров[Проверочные_векторы[3].параметры],
			false
		)) == this.ЭЦП.Согласование_ключей(
			Число_в_код(Проверочные_векторы[3].B_закр, 74),
			Двойное_число_в_код(...Проверочные_векторы[3].A_откр, true),
			Проверочные_векторы[3].UKM,
			this.ЭЦП.Наборы_параметров[Проверочные_векторы[3].параметры],
			false
