...
class ГОСТ_Хэш {
	...
	static #H(IV, M) {
		...
		let
			...
			длина = M.length * 8;
		while (длина >= 512) {
			for (let i = 0; i < 64; i++) m[i] = M[i + (длина >>> 3) - 63 - ((длина & 0x7) == 0)];
			h = this.#g_N(N, h, m);
	...
	static Вычислить(
		...
		if (typeof данные == "string") {
			const код = [];
			for (let сч = 0; сч < данные.length; сч++) {
				const символ = данные.charCodeAt(сч);
				код.push((символ & 0xFF00) >>> 8);
				код.push(символ & 0xFF);
			}
			return длинный ? this.#Хэш_512(код) : this.#Хэш_256(код);
		}
		else
