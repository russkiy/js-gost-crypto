...
class ГОСТ_Шифр {
	...
	static #В_массив_байтов(значение, длина) {
		let массив = [];
		if (typeof значение == "number") {
			if (значение < 0) значение += 0x100000000;
			while (значение > 0) {
				массив.push(значение & 0xFF);
				значение >>>= 8;
			}
		}
		else
			for (let сч = 0; сч < значение.length; сч++) {
				const код = значение.charCodeAt(сч);
				массив.push(код & 0xFF, код >>> 8);
			}
		if (длина !== undefined && массив.length < длина)
			массив = массив.concat(new Array(длина - массив.length).fill(0));
		return массив;
	}

	static #Из_массива_байтов_в_число(массив) {
		let число = 0;
		for (let сч = 0; сч < массив.length; сч++)
			число += массив[сч] << 8 * сч;
		return число;
	}

	static #Из_массива_байтов_в_строку(массив) {
		let строка = "";
		for (let сч = 0; сч < массив.length; сч += 2)
			строка += String.fromCharCode(массив[сч] + (массив[сч + 1] << 8));
		return строка;
	}
	...
	static Простая_замена( // режим простой замены
		...
		const в_виде_строки = !Array.isArray(T);
		if (в_виде_строки) T = this.#В_массив_байтов(T);

		const результат = [];
		for (let сч = 0; сч < T.length; сч += 8) {
			let N = [
				this.#Из_массива_байтов_в_число(T.slice(сч, сч + 4)),
				this.#Из_массива_байтов_в_число(T.slice(сч + 4, сч + 8))
			];
			N = D ? this.#Расшифрование(N, X, K) : this.#Зашифрование(N, X, K);
			N.forEach(n => this.#В_массив_байтов(n, 4).forEach(б => результат.push(б)));
		}

		return в_виде_строки ? this.#Из_массива_байтов_в_строку(результат) : результат;
	}
	...
	static Гаммование( // режим гаммования
		...
		for (let сч = 0; сч < T.length; сч += 8) {
			...
			const
				...
				N = [
					this.#Из_массива_байтов_в_число(T.slice(сч, сч + 4)) ^ Г[0],
					this.#Из_массива_байтов_в_число(T.slice(сч + 4, сч + 8)) ^ Г[1]
				];
			let осталось = T.length - сч;
			const байты = this.#В_массив_байтов(N[0], 4).concat(this.#В_массив_байтов(N[1], 4));
			for (let сч_б = 0; сч_б < (осталось < 8 ? осталось : байты.length); сч_б++)
				результат.push(байты[сч_б]);
		}
	...
	static Гаммование_с_обратной_связью( // режим гаммования с обратной связью
		...
		for (let сч = 0; сч < T.length; сч += 8) {
			...
			const
				T_0 = this.#Из_массива_байтов_в_число(T.slice(сч, сч + 4)),
				T_1 = this.#Из_массива_байтов_в_число(T.slice(сч + 4, сч + 8));
			S[0] = S[0] ^ T_0;
			S[1] = S[1] ^ T_1;
			let осталось = T.length - сч;
			const байты = this.#В_массив_байтов(S[0], 4).concat(this.#В_массив_байтов(S[1], 4));
			for (let сч_б = 0; сч_б < (осталось < 8 ? осталось : байты.length); сч_б++)
				результат.push(байты[сч_б]);
			if (D) {
				S[0] = T_0;
	...
	static Имитовставка( // режим выработки имитовставки
		...
		X,	// ключ (массив[8 <32-битных чисел>])
		K	// таблица замен (массив[8 * 16 <4-битных чисел>])
	) { // возвращает имитовставку (строку[4])
		if (!K) K = this.Стандартные_ТЗ[this.#Таблица_по_умолчанию];
		const в_виде_строки = !Array.isArray(T);
		if (в_виде_строки) T = this.#В_массив_байтов(T);
		...
		let S = [0, 0];
		for (let сч = 0; сч < T.length; сч += 8) {
			S[0] = this.#Из_массива_байтов_в_число(T.slice(сч, сч + 4)) ^ S[0];
			S[1] = this.#Из_массива_байтов_в_число(T.slice(сч + 4, сч + 8)) ^ S[1];
			S = this.#Выработка_имитовставки(S, X, K);
		}
		const результат = this.#В_массив_байтов(S[0], 4).concat(this.#В_массив_байтов(S[1], 4));
	...
	static Сгенерировать_синхропосылку() { // возвращает синхропосылку (массив [2 <32-битных числа>])
		const S = [];
		for (let i = 0; i < 2; i++)
			S[i] = Math.random() * 0x100000000 | 0;
		this.Синхропосылка = S;
	}
	...
	// число в ключ или синхропосылку
		...
		размер_блока = BigInt(размер_блока);
		const массив = [];
		while (массив.length < количество_блоков) {
			массив.push(Number(число & ((1n << размер_блока) - 1n)));
			число >>= размер_блока;
		}
		return массив;
	}
